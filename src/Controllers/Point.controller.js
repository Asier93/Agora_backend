...
export const point = async (req, res) => {
          ...
          const block = await Block.findOne({ _id: infoOption.blockId.toString() }).exec();
          const category = await Category.findOne({ _id: infoOption.categoryId.toString() }).exec();
          ...
          if (category.name == 3) {
                    ...
                    const optionIds = await Option.find({ blockId: block._id }).select('_id').exec();
                    const optionIdList = optionIds.map(option => option._id);
                    const pointsBlock = await Point.find({ gameId: game, optionId: { $in: optionIdList } })
                    .populate("optionId")
                    .exec();
                    ...
                    const optionsBlock = await Option.find({blockId: block._id.toString(), categoryId: '663df7d5fc567fca9153cae5' }).exec();
                    //console.log('Opciones del bloque 3: ' + optionsBlock) 
                    // Filtro para saber cúal no se ha seleccionado del nivel 3
                    const missingOptions = optionsBlock.filter(option => !uniqueOptionNamesTrim.includes(option.name.trim()));
